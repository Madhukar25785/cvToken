...
const MatrixIncome = () => {
  ...
  const fetchData = async () => {
    try {
      const request = await _fetch(`${base_url}/api/level-income`, 'GET', {}, {})
      // console.log("level-income", request);
      setMatrixIncome(request.data)
      ...
      <div className='col-md-12 mt-5'>
        <div className='table-responsive'>
          ...
          <table className="mt-table mt-2">
            ...
            <tbody>
              {matrixIncome && matrixIncome.map((item, index) => {
                return (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>{item.Holding}</td>
          ...
          <div className='market-table-footer'>
            {matrixIncome && matrixIncome.length > 0 ? (
              <>
